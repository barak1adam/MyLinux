...
static int mv_eth_indirect_smi_plat_data_get(struct device_node *mdio_node, struct mv_neta_pdata *plat_data)
{
	...
	in_addr = htonl(smi_iobase_size[0]);

	smi_iobase_size[0] = of_translate_address(mdio_node, &in_addr);
...
static struct mv_neta_pdata *mv_plat_data_get(struct platform_device *pdev)
{
	...
	/* fix speed for OS-V20-Caelum device.
	 * for OS-V20 Caelum Cost Reduction, where SOHO is not present on board, don't fix it !! */
	if (plat_data->ctrl_model == 0xbc00 && plat_data->phy_addr == -1)
...
static int mv_eth_probe(struct platform_device *pdev)
{
	...
	pr_info("================\n\r");
	pr_info("plat_data.tclk = %d\n\r", plat_data->tclk);
	pr_info("plat_data.pclk = %d\n\r", plat_data->pclk);
	pr_info("plat_data.max_port = %d\n\r", plat_data->max_port);
	pr_info("plat_data.max_cpu = %d\n\r", plat_data->max_cpu);
	pr_info("plat_data.ctrl_model = %d\n\r", plat_data->ctrl_model);
	pr_info("plat_data.ctrl_rev = %d\n\r", plat_data->ctrl_rev);
	pr_info("plat_data.cpu_mask = %d\n\r", plat_data->cpu_mask);
	pr_info("plat_data.mtu = %d\n\r", plat_data->mtu);
	pr_info("plat_data.phy_addr = %d\n\r", plat_data->phy_addr);
	pr_info("plat_data.is_oob_phy = %d\n\r", plat_data->is_oob_phy);
	pr_info("plat_data.oob_phy_smi_base = %d\n\r", plat_data->oob_phy_smi_base);
	pr_info("plat_data.oob_phy_smi_remap_size = %d\n\r", plat_data->oob_phy_smi_remap_size);
	pr_info("plat_data.tx_csum_limit = %d\n\r", plat_data->tx_csum_limit);
	pr_info("plat_data.speed = %d\n\r", plat_data->speed);
	pr_info("plat_data.duplex = %d\n\r", plat_data->duplex);
	pr_info("plat_data.lb_enable = %d\n\r", plat_data->lb_enable);
	pr_info("plat_data.is_sgmii = %d\n\r", plat_data->is_sgmii);
	pr_info("plat_data.is_rgmii = %d\n\r", plat_data->is_rgmii);
	pr_info("plat_data.irq = %d\n\r", plat_data->irq);
	pr_info("plat_data.rx_queue_count = %d\n\r", plat_data->rx_queue_count);
	pr_info("plat_data.tx_queue_count = %d\n\r", plat_data->tx_queue_count);
	pr_info("plat_data.rx_queue_size = %d\n\r", plat_data->rx_queue_size);
	pr_info("plat_data.tx_queue_size = %d\n\r", plat_data->tx_queue_size);
	pr_info("plat_data.pnc_tcam_size = %d\n\r", plat_data->pnc_tcam_size);
	pr_info("plat_data.MAC_ADDR = %X%X%X%X%X%X\n\r", plat_data->mac_addr[0], plat_data->mac_addr[1], plat_data->mac_addr[2], plat_data->mac_addr[3], plat_data->mac_addr[4], plat_data->mac_addr[5]);
...
int mv_eth_hal_init(struct eth_port *pp)
{
	...
	if (!MV_PON_PORT(pp->port)) {
		...
		/* internal SMI: used with HW polling via Gbe MAC, need to enable HW PHY polling */
		if (pp->plat_data->is_oob_phy) {
			/* Barak: is_oob_phy is set if PHY is directly connected to CPU */

			/* if OS-V8, SoC model = CETUS */
			if (pp->plat_data->ctrl_model == 0xbe00){
				pr_info("Neta port %d: Enable HW SMI polling of PHY at 0x%08x\n",
					pp->port, phyAddr);
				mvNetaPhyAddrSet(pp->port, phyAddr);
			}
			/* if XG312, SoC model is BC2 */
			else{
		...
		/* Barak: is_oob_phy is NOT set if PHY is connected by a SOHO switch, like OS-V20 where SoC model = Caelum */
			pr_info("Neta port %d: Disable HW SMI polling of PHY\n", pp->port);
			/* If not OOB port, do not poll PHY via SMI. */
			mvNetaPhyAddrPollingDisable(pp->port);
		}
...
static void mv_eth_adjust_link(struct eth_port *pp)
{
	...
	/*read out phy's link*/
	pr_err("Barak %s %d: read out phy's link\n", __func__, __LINE__);
	pr_err("Barak %s %d: Port %d mvEthPhyCheckLink\n", __func__, __LINE__, pp->port);
	phy_link = mvEthPhyCheckLink(pp->plat_data->phy_addr);

	if (phy_link) {
		/*read out phy's speed and duplex*/
		pr_err("Barak %s %d: read out phy's speed and duplex\n", __func__, __LINE__);
		pr_err("Barak %s %d: Port %d mvEthPhySpeedDuplexGet\n", __func__, __LINE__, pp->port);
		...
		if ((pp->speed != phy_speed) ||
			...
			pr_err("Barak %s %d: if link up, check autoneg result change and update autoneg results to eth port\n", __func__, __LINE__);
			pr_err("Barak %s %d: Port %d mvEthPhySpeedDuplexGet\n", __func__, __LINE__, pp->port);
	...
	mvEthPhyRegs(pp->plat_data->phy_addr);
	mvEthPhyPrintStatus(pp->plat_data->phy_addr);
}

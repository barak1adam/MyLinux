		    ...
		    IN void *addiatinal_param)
{
  ...
  case VIF_EVNT_VIF_CHANGE:
    ...
    /* UID_IF_EVENT_IF_ADDED: tag + ports are set to VIF: enable VRRP only if VIF is UP
     * on startup it's sent but VIF is not yet UP and VRRP is not yet set.
     * UID_IF_EVENT_IF_ADDED: if VIF is enabled by the user: enable VRRP only if VIF is UP
     * 	if VIF is not UP, no need to enable VRRP. 
     * 	If VIF is not UP but changed (ports, IPs are added or removed), it is covered by UID_IF_EVENT_IF_MODIFIED */
    case UID_IF_EVENT_IF_ADDED:
       ...
       break;
    /* UID_IF_EVENT_IF_MODIFIED: ports or IPs are set to VIF (added or removed)
     *  on startup it's sent but VRRP is not set yet
     *  if VIF is changed, then restart VRRP to check changes like ownership. */
    case UID_IF_EVENT_IF_MODIFIED:
    case UID_IF_EVENT_IF_IPv6_ADD:
    case UID_IF_EVENT_IF_IPv6_DEL:
	if (OK == (status = vrrp_get_if (&uid_vrrp_if))) {
	    /* Upon restart, VRs are remove and added again to conf file and keepalived is re-invoked */
    ...
    case UID_IF_EVENT_IF_STATE_CHANGED:
      /*   DO NOTHING.
           When interface state is changed, Keepalived gets notifiications by netlink.    */
#if 0
	if (OK == (status = vrrp_get_if (&uid_vrrp_if))) {
	    /* Upon restart, VRs are remove and added again to conf file and keepalived is re-invoked */
	    if (OK != (status = vrrp_restart_if (ui_if_ptr->dev))) {
	    ...
	    INOUT char            *buff/*[MAX_REQ_BUFF_SIZE]*/)
{
  ...
  case UID_VRRP_ENABLE_IF:
      /* when VRRP is enabled on an Interface, it's VRRP state is set to ENABLE or to ACTIVE if the underlying IF is ACTIVE */
      /* VRs are started in Keepalived (added to conf file and reloaded), only if VRRP is in ACTIVE state ! */
      GEN_LOG_log_console("%s: start if \n\r", __FUNCTION__);
  ...
  case UID_VRRP_DISABLE_IF:
      /* when VRRP is disabled on an Interface, it's VRRP state is set to DISABLE */
  ...
  case UID_VRRP_STOP_IF:
      GEN_LOG_log_console("%s: stop if \n\r", __FUNCTION__);
      /* when VRRP is stopped on an Interface, it's VRRP state is set to ENABLE */

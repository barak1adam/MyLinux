...
static int stm32_spi_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "driver probe started\n");

	spi->irq = platform_get_irq(pdev, 0);
	if (spi->irq <= 0) {
		ret = spi->irq;
		if (ret != -EPROBE_DEFER)
			dev_info(&pdev->dev, "failed to get irq: %d\n", ret);
		goto err_master_put;
	}
	...
	if (ret) {
		dev_info(&pdev->dev, "irq%d request failed: %d\n", spi->irq,
			ret);
		goto err_master_put;
	}
	...
	if (IS_ERR(spi->clk)) {
		ret = PTR_ERR(spi->clk);
		dev_info(&pdev->dev, "clk get failed: %d\n", ret);
		goto err_master_put;
	}

	dev_info(&pdev->dev, "driver probe started 1\n");

	ret = clk_prepare_enable(spi->clk);
	if (ret) {
		dev_info(&pdev->dev, "clk enable failed: %d\n", ret);
		goto err_master_put;
	}
	spi->clk_rate = clk_get_rate(spi->clk);
	if (!spi->clk_rate) {
		dev_info(&pdev->dev, "clk rate = 0\n");
		ret = -EINVAL;
		goto err_clk_disable;
	}

	dev_info(&pdev->dev, "driver probe started 2\n");
	...
	if (rst) {
		if (IS_ERR(rst)) {
			ret = PTR_ERR(rst);
			if (ret != -EPROBE_DEFER)
				dev_info(&pdev->dev, "reset get failed: %d\n",
					ret);
	...
	if (ret) {
		dev_info(&pdev->dev, "controller configuration failed: %d\n",
			ret);
		goto err_clk_disable;
	}

	dev_info(&pdev->dev, "driver probe started 3\n");
	...
	if (IS_ERR(spi->dma_tx)) {
		...
		if (ret == -EPROBE_DEFER)
			...
			dev_info(&pdev->dev, "failed to request tx dma: %d\n",
				 ret);
	}
	...
	if (IS_ERR(spi->dma_rx)) {
		...
		if (ret == -EPROBE_DEFER)
			...
			dev_info(&pdev->dev, "failed to request rx dma: %d\n",
				 ret);
	}
	...
	dev_info(&pdev->dev, "driver probe started 4\n");
	...
	for (i = 0; i < num_cs; i++) {
		...
		if (gpio_is_valid(cs_gpio)) {
			ret = devm_gpio_request(&pdev->dev, cs_gpio,
						DRIVER_NAME);
			if (ret) {
				dev_info(&pdev->dev, "can't get CS gpio %i\n",
					cs_gpio);
				goto err_dma_release;
			}
		}
	...
	dev_info(&pdev->dev, "driver probe started 5\n");
